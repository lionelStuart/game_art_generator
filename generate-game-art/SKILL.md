...
4. Generate image variations via Codex `image_gen`.
...
## Codex image_gen Integration Rule

Use a renderer callback that calls Codex built-in `image_gen` and returns image bytes for each variation. Keep parsing, style, prompt, and validation modules unchanged.
...
```bash
python3 scripts/generate_game_art.py --input /path/to/request.json --output-root outputs --use-codex-image-gen
```

Script behavior in MVP:

- Generates structured output following `GenerateGameArtOutput`.
- Writes image bytes to expected naming paths.
